...
public interface AlertRepository extends JpaRepository<Alert, Long> {
    ...
    @Query("SELECT COUNT(a)>0 FROM Alert a WHERE a.user.id=:userId AND (:categoryId IS NULL AND a.category IS NULL OR a.category.categoryId=:categoryId) AND a.alertType=:type AND a.month=:month AND a.year=:year")
