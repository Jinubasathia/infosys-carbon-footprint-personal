...
public class ProfileUpdateRequest {
    ...
    @NotNull(message = "Age is required") @Min(value = 18, message = "User must be at least 18 years old") @Max(value = 120, message = "Invalid age") private Integer age;
    @NotNull(message = "Gender is required") private Gender gender;
    @NotNull(message = "Date of birth is required") @Past(message = "Date of birth must be in the past") private LocalDate dateOfBirth;
    @NotBlank(message = "Mobile number is required") @Pattern(regexp = "^[6-9]\\d{9}$", message = "Mobile number must be a valid 10-digit Indian phone number") private String mobileNumber;
    ...
    @NotNull(message = "Address details are required") @Valid private AddressDto address;
}