...
import { User, MapPin, FileCheck, ArrowRight, ArrowLeft, CheckCircle2, AlertCircle } from 'lucide-react';
...
const RegisterPage = () => {
  ...
  const validateStep3 = () => {
    const errors = {};
    if (!formData.governmentId.idNumber.trim()) {
      errors['governmentId.idNumber'] = 'Government ID number is required';
    }
    setFieldErrors(errors);
    return Object.keys(errors).length === 0;
  };
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-300 mb-1">ID Document Number *</label>
                    <input
                      type="text"
                      name="idNumber"
                      ...
                      className="w-full px-3.5 py-2.5 rounded-lg bg-slate-950 border border-slate-800 focus:border-emerald-500 text-sm text-white outline-none font-mono"
                    />
                    {fieldErrors['governmentId.idNumber'] && <span className="text-xs text-rose-400 mt-1 block">{fieldErrors['governmentId.idNumber']}</span>}
                  </div>
                </div>
